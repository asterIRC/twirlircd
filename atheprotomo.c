...
static void inspircd_join_sts(channel_t *c, user_t *u, bool isnew, char *modes)
{
	if (isnew)
	{
		sts(":%s FJOIN %s %lu + :ao,%s", me.numeric, c->name, (unsigned long)c->ts, u->uid);
		if (modes[0] && modes[1])
...
{
	sts(":%s PRIVMSG %s :%s", from ? from->uid : me.numeric, target->name, text);
}
...
static void inspircd_on_login(user_t *u, myuser_t *mu, const char *wantedhost)
{
	...
	if (should_reg_umode(u)) sts(":%s SVSMODE %s +r", me.numeric, u->uid);
}
